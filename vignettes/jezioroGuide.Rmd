...
Two vectors are required for input data (x=the independent variable, y=the dependent variable), and the arguments *xLabel*, *yLabel*, and *yRange*, are optional and simply allow customization of both axis labels and the range of the y-axis to ensure they remain consistent across the produced plots.

For example, using the example chlorophyll *a* profile provided in `vrsChlaInput`, fitting a GAM between sediment depth (x) and the inferred-chlorophyll *a* values (y) can be done as follows:
...
The three optional arguments can be used to provide consistency across each of the output plots.
```
...
If you aren't interested in using the optional arguments, using 'drop=FALSE' when reading in the data can be used to preserve column names for use in the plots.

```
quickGAM(plotData[,1, drop=FALSE],plotData[,2, drop=FALSE], xLabel = indepVarLabel, yLabel = depenVarLabel, yRange = depenVarRange)
```

The function will produce six plots:

- A simple scatterplot of the input data

- Estimated CAR1 process from the GAM fitted to the input data ( i.e. an autocorrelation check)

- GAM-based trends fitted to the input data

- Estimated trends (w 20 random draws of the GAM fit to the input data)
...
- new function: `quickGAM` (a function to fit a GAM and perform the related analyses described in Simpson (2018)))

## v0.19 (Sept 2019)
**General**

- the package is now hosted in a [github repository](https://github.com/shiggo/jezioro)
