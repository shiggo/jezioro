...
```
If you are dealing with a large number of variables, it may make examine all of them at once using the `apply` function.
```
...
``` {r}
crossval(vwho.model, cv.method="loo", verbose=FALSE, ngroups=10, nboot=100, h.cutoff=0, h.dist=NULL)
```
