...
# Introduction
Despite countless blog posts, textbooks, and online courses on R already littering the internet, this is an attempt at an overview of how R is typically used in the paleolimnological analyses performed at [PEARL](http://post.queensu.ca/~pearl/). 

The purpose of the guide is to aid both new and experienced users refine the many inevitable internet searches prefaced with [*"r help ..."*](
https://www.google.ca/search?q=r+help&oq=r+help&aqs=chrome..69i57.1854j0j8&sourceid=chrome&ie=UTF-8).

Although the guide begins with basic concepts and usage of R, the majority of its content is focused on specific packages and functions commonly used by members of PEARL. The guide was created using [R Markdown](http://rmarkdown.rstudio.com/), specifically to allow easy updates and its gradual evolution over time. Therefore, please share any suggestions you may have that can be incorporated and improve future versions.
...
## Ordination
The tools necessary for performing ordinations are provided by the `vegan` package.

A DCA can be performed using the function `decorana`
Somewhat confusingly, both PCAs and RDAs are performed using the function `rda` (If only one input matrix is provided it will run a PCA, if two are provided it will run an RDA)
Biplots can be produced from an rda object by the function `biplot`
...
## Analog Matching
When comparing fossil data with a calibration set (i.e. when using a transfer function), identifying whether there are close modern analogs present in the calibration set can be done using the `analog` function provided by the `analogue` package.
